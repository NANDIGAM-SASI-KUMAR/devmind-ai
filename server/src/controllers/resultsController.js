...
export const getTrend = async (req, res) => {
  ...
  res.json({ hasData: points.length > 0, points });
};

// GET /api/results/topics
export const getTopics = async (req, res) => {
  const { quizAttempts, examAttempts } = await loadAttempts(req.user._id);
  if (quizAttempts.length === 0 && examAttempts.length === 0) return res.json({ hasData: false, topics: [], strong: [], weak: [] });

  const topics = computeTopicPerformance(quizAttempts, examAttempts);
  const { strong, weak } = classifyStrongWeak(topics);
  res.json({ hasData: true, topics, strong, weak });
};
...
export const getTopicDetail = async (req, res) => {
  ...
  if (!detail) return res.status(404).json({ message: 'No data for this topic yet' });
...
export const getQuizAnalytics = async (req, res) => {
  ...
  if (quizAttempts.length === 0) return res.json({ hasData: false });
...
export const getExamAnalytics = async (req, res) => {
  ...
  if (examAttempts.length === 0) return res.json({ hasData: false });
